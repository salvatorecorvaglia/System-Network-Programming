...
int main(int argc, char *argv[]){
   ...
   printSockaddr_in((struct sockaddr *) &sa);
	...
	ssize_t n = 0;
		...
		snprintf(msg, BUFSIZE-1, "%s", argv[1]);
...
return 0;
		...
		m = read(fd[0], bufpipe,	256);
		printf("\n Letto dalla pipe: %s", bufpipe);
		execlp("./MulticastSender","MulticastSender","224.0.0.1", "48150", "ciao", NULL);
